...
public class ConnectionGuesser
{
	...
	@Inject
	public ConnectionGuesser(DataManager dataManager, TextParser textParser, WifiDataManager wifiManager){
	...
	public Observable<ConnectionAttempt> guess(Observable<TextParserResponse> textParseResult/*, boolean fuzzyPassword*/){
		...
		return Observable.combineLatest(textParseResult,
			networksAvailable, new BiFunction<TextParserResponse, List<WifiNetwork>, List<ConnectionAttempt>>(){
				@Override
				public List<ConnectionAttempt> apply(TextParserResponse tpr,
											   List<WifiNetwork> wifiNetworks) throws Exception{
			...
			(tpr, wifiNetworks) -> {
				...
				if(tpr.hasBoth()){
					...
					if(!GenUtils.isBlank(verifiedSSID)){
						...
						//Timber.w("SSID was not found: %s (trying anyway)", tpr.getSsid());
						//return Collections.singletonList(new ConnectionAttempt(tpr.getSsid(), tpr.getPassword()));
						Timber.warn("Unable to find wifi network '" + tpr.getSsid() + "'");
						//throw new Exception("Unable to find wifi network '" + tpr.getSsid() + "'");
					}
				}
				else if(tpr.hasPassword()){
